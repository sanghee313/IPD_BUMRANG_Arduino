...
void loop() {
  ...
  if(digitalRead(Drink_Switch)==LOW)
  {
   ...
   Serial.println("Drink_Pump_ON");
  }
  else
  {
  digitalWrite(Drink_Relay,LOW);
  Serial.println("Drink_Pump_OFF");  
  }
  ...
  {  
    ...
    {  
      Serial.println("FIRST_TANK2_FULL");
      Serial.println("DO_SPIN_MOTOR");
      digitalWrite(SPIN_PIN, HIGH); //릴레이 접점 ON -> 물채우기
      MotorState = true;
      delay(5000); //5초 대기
      digitalWrite(SPIN_PIN, LOW); //릴레이 접점 OFF
      Serial.println("STOP_SPIN_MOTOR");
      MotorState = false;
      MotorCount++;
    ...
    { 
      Serial.println("ALREADY DO SPIN");
      MotorCount++;  
    } 
  }
